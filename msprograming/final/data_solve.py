...
coff_dir = [-30, 10, 20, 20, 30, -30, 25, 30, 0]
coff_speed = [0, -5, -40, -60, -70, -75, -80, -80, -90, -90, -65, -75]
...
for FILE in dir_list:
	...
	filename = refind[0]
	reader = open(FILE, 'r')
	writer = open(os.path.join('out', filename+'.csv'), 'w')
	AQI_list = []
	the_list = []
	...
	for ttt in range(63):
		...
		curAQI = the_list[0].AQIlevel * 100 + random.randint(-60, 60)
	...
	#	curAQI += get_best_add()
		...
		bad = 0
		normal = 0
		day = datetime.datetime.strptime(the_list[0].date, "%Y-%m-%d")
		if curtime < 4:
		...
		for tt in range(12):
			...
			curAQI += add
			curAQI = max(curAQI, 0)
			condition = get_condition(curAQI)
			if condition == 'good':
				good += 1
			if condition == 'bad':
				bad += 1
			if condition == 'normal':
				normal += 1
			curtime = (curtime + 1) % 24
		total = good + bad + normal + 0.0
		lines = ['%d-%02d-%02d' % (day.year, day.month, day.day) + ' ', str(ttime) + ',', '%.3f' %(good / total) + ',', '%.3f' %(normal / total) + ',', '%.3f' %(bad / total), '\n\n']
