// File Name: B.cpp
// Author: YangYue
// Created Time: Sun Jun 29 00:25:27 2014
//headers 
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
public:
	int n;
	vector<PII> edges[55];
	long long A[55][55], B[55][55], tmp[55][55];
	long long findMin(int N, vector <int> from, vector <int> to, vector <int> weight, int charges) {
		...
		for (int i = 0; i < m; ++i) {
			--from[i];
			--to[i];
			A[from[i]][to[i]] = min(A[from[i]][to[i]], (long long)weight[i]);
		}
		for (int k = 0; k < n; ++k)
		for (int i = 0; i < n; ++i)
		...
		for (int j = 0; j < n; ++j)
		for (int k = 0; k < m; ++k) {
			A[i][j] = min(A[i][j], B[i][from[k]] + B[to[k]][j] - weight[k]);
		}
		for (int t = charges; t; t >>= 1) {
			if (t & 1) {
				memcpy(tmp,B,sizeof(B));
				for (int k = 0; k < n; ++k)
				for (int i = 0; i < n; ++i)
				for (int j = 0; j < n; ++j) 
					B[i][j] = min(B[i][j], tmp[i][k] + A[k][j]);
			}
			memcpy(tmp,A,sizeof(A));
			for (int k = 0; k < n; ++k)
			for (int i = 0; i < n; ++i)
			for (int j = 0; j < n; ++j)
				A[i][j] = min(A[i][j],tmp[i][k] + tmp[k][j]);
		}
		return B[0][n-1];
	}
};
