...
public:
	...
	long long dist[1111];
	vector<pii> edges[1111], backedges[1111];
	long long dp(int u, int K) {
		...
		for (vector<pii> :: iterator it = edges[u].begin(); it != edges[u].end(); ++it) {
			if (K) {
				tmp2 = max(tmp2, dp(it->first, K-1) + it->second);
			}
			if (dist[u] > dist[it->first]) {
				contains.push_back(*it);
			}
		}
		if (contains.size() || K == 0) {
			long long tmp = INF;
			for (vector<pii> :: iterator it = contains.begin(); it != contains.end(); ++it) {
				tmp = min(tmp, dp(it->first, K) + it->second);
			}
			tmp2 = max(tmp2, tmp);
		}
		f[u][K] = tmp2;
	...
	long long minPath(int N, int K, vector <int> from, vector <int> to, vector <int> cost) {
		...
		for (int i = 0; i < from.size(); ++i) {
			edges[from[i]].push_back(make_pair(to[i], cost[i]));
			backedges[to[i]].push_back(pii(from[i], cost[i]));
		}
		SPFA(N-1, N);
		memset(f, -1, sizeof f);
		for (int j = 0; j <= K; ++j) f[N-1][0] = 0;
		cout << dp(0, K) << endl;
		return dp(0, K);
	}
};
