...
f[i][j]表示前i个数，加上1 - A[j]之后在[1..n]区间的有多少个数,求出f[][]数组时候用f[i][j] + (max{f[p][x]} - f[p][j])(1<=x<=n) 更新答案即可，复杂度O(n^2)
...
采用大-小分治，取S=[sqrt(2n)]，对于度数>S的点（称为大点）暴力维护邻接边的正权和、负权和，对于度数<=S的点（称为小点）每次修改暴力修改每一条邻接边。大点之间的边（S^2条）暴力维护。
